...
class LagrangianParticles:
    ...
    def locate(self, particle):
        ...
        if isinstance(particle, Particle):
            ...
            return self.tree.compute_closest_entity(point)[0]
        else:
            return self.locate(Particle(particle))


    def get_particles(self, timestep):
...
class ParticlesFromPoint():
    def __init__(self, point):
        self.point = point

    def generate(self, N):
        sample_x = np.random.normal(self.point[0], 0.01, N)
        sample_y = np.random.normal(self.point[1], 0.01, N)
        sample_z = np.random.normal(self.point[2], 0.01, N)

        points = (np.array([sample_x[i], sample_y[i], sample_z[i]]) for i in range(N))
        return points
